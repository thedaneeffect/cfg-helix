...
./setup.sh go        # Configure GOPATH and install Go tools (gopls, golangci-lint, delve, air)
...
- **Dependency Management**: Installs Homebrew, yq, helix, go, fzf, zoxide, direnv, task, ripgrep, ast-grep, fd, bat, eza, delta, jq, btop, tldr, sd, glow, tokei, gh, procs, dust, and usql if missing
...
- **Go Tools**: Installs gopls (language server), golangci-lint (linter), delve (debugger), and air (live reload) via go install
